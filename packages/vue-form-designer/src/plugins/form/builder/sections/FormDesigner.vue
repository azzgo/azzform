<template>
  <a-row class="container" type="flex">
    ...
    <a-col flex="auto" class="content">
      <editor-preview-area @selectFieldChange="selectFieldId = $event"></editor-preview-area>
    </a-col>
    <a-col flex="300px" class="sider">
      <editor-panel
...
import EditorPreviewArea from "../components/EditorPreviewArea.vue";
...
export default {
  computed: {
    activedField() {
      return state.schema?.properties?.[state.designer.selectFieldId];
    },
  },
  data() {
    return {
  ...
  methods: {
    handleSettingChange(val) {
      state.schema.properties[state.designer.selectFieldId] = val;
    },
  },
  components: {
    Toolbar,
